...
export default class Retreats extends Component {
  constructor(props){
    super(props)
    this.state={TTC:false,BUTTON:3,Link:this.props.lang+"/Retreat"};
    console.log(this.state.BUTTON);
  }
  render() {
    const HandleTTC1=()=>{
      this.setState({...this.state,TTC:true,BUTTON:4,Link:""});
    }
    const HandleTTC2=()=>{
      this.setState({...this.state,TTC:false,BUTTON:3,Link:this.props.lang+"/Retreat"});
  ...
  transition={{delay:0.2}}>
          ...
          <div className='Retreats-buttons'>
                      <Link to={lang+"/Retreat"}><Button color="white" background="linear-gradient(30deg,#fb2f7b, #ff845d)"  Link="/"><h3 style={{color:"white"}}>{LANG[this.props.lang][this.state.BUTTON]}</h3></Button></Link>
          </div>
        </div>
      </div>
    )
